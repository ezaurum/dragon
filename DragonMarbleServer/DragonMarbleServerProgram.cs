    ...
    public class DragonMarbleServerProgram
    {
        ...
        private static void ConvertBytesToMessage(object sender, SocketAsyncEventArgs eventArgs)
        {
            ...
            IGameMessage gameMessage = new GameMessage()
            {
                ...
                {
                    MessageLength = messageLength, From = new Guid(m.Skip(GameMessageHeader.FirstGuidIndex).Take(16).ToArray()), To = new Guid(m.Skip(GameMessageHeader.SecondGuidIndex).Take(16).ToArray())
                }
            };
            ...
            if (gameMessageType == GameMessageType.Roll)
            {
                ...
                {
                    MessageType = gameMessageType, Content = new RollMoveDiceContentC2S()
                    {
